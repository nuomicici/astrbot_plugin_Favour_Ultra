...
| 命令格式                               | 功能描述                                   | 权限要求       | 示例                                 |
|----------------------------------------|--------------------------------------------|----------------|--------------------------------------|
| `清空当前好感度`                       | 清空当前会话的好感度数据（需二次确认）     | 群主及以上     | 发送「清空当前好感度」               |
| `清空全局好感度数据`                   | 清空所有会话的好感度数据（需二次确认）     | Bot管理员      | 发送「清空全局好感度数据」           |
| `删除好感度数据 <用户ID>`              | 删除指定用户的好感度数据（按当前会话模式） | 群管理员及以上 | 发送「删除好感度数据 123456789」     |
| `修改好感度 <用户ID> <数值>`           | 修改指定用户的好感度（数值范围：-100~100） | 群管理员及以上 | 发送「修改好感度 123456789 80」      |
| `查询好感度数据`                       | 查看当前会话所有用户的好感度、关系信息     | 群管理员及以上 | 发送「查询好感度数据」               |
| `查询全部好感度`                       | 查看所有会话的好感度、关系、会话信息       | Bot管理员      | 发送「查询全部好感度」               |
...
4. **权限等级说明**：
   ...
   - 高等级成员：群等级达到阈值的成员（当前阈值：level_threshold）
   - 普通用户：默认权限，仅可查看自身好感度
...
### Q5：如何升级到v2.0版本？
A5：
1. 备份旧版本数据文件（`haogan.json`和`global_favour.json`）
2. 停止旧版本插件
3. 安装新版本插件
4. 将备份文件复制到新版本数据目录（`./data/hao_gan_du/`）
5. 启动新版本插件，系统会自动完成数据迁移
