<div align="center">
<img style="width:70%" src="https://count.getloli.com/@astrbot_plugin_Favour_Ultra?name=astrbot_plugin_Favour_Ultra&theme=booru-lewd&padding=6&offset=0&align=top&scale=1&pixelated=1&darkmode=auto" alt="Banner">

# 好感度/关系管理系统 (Favour Ultra)

[![当前版本](https://img.shields.io/badge/当前版本v4.4.5-blue?style=for-the-badge)](#更新日志) 
...
* **WebUI 全面重构与美术升级 (v4.4.5)**：采用 Neo-Bento 模块化设计与长春藤紫/澄澈冰蓝冷色调方案；支持好感分级连续谱（Spectrum Bar）可视化；定制浮动下拉选单组件；数据中心支持私聊/群聊同级折叠与多维动态排序；顶栏标题增加动态流光动效；动态读取并展示插件真实版本号。
* **优化注入 prompt，节省 token 和加强输出标签限制 (v4.4.4)**：轻量化重构提示词，利用近因强化杜绝长对话漏标，提升 RP 人设专注度。
* **配置不再清空 (v4.4.3)**：修复插件更新/重启后配置可能被默认配置覆盖的问题（原子写入 + 损坏自动恢复）。
* **搭话修复与平台扩展 (v4.4.0~v4.4.2)**：直接发送路径注入对话历史上下文；发送前去除好感度标签；全面适配支持主动消息的适配器；修复私聊未触发与白名单模糊匹配。
* **会话迁移与双向完全同步 (v4.3.0)**：支持多会话/WebChat 好感度一键复制迁移与实时双向同步。
...
*   **上下文精准注入**：基于 `<FavourContext>` 结构，实时将好感度等级、好感上限、现有关系及排他约束注入上下文，确保模型对话的人设稳定性。
...
插件会实时解析当前交互对象，并将状态通过 `<FavourContext>` 节点注入动态用户上下文。这包含了当前交互对象的 ID、管理权限、好感度数值与等级、已建立的单边/排他性关系等信息，甚至包含了对当前好感度上限的动态约束声明与末行日志强制提示。系统借此引导大模型在生成回复时做出最符合当前关系阶段与身份设定的选择。
